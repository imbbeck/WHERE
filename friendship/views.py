...
def friendship_add_friend(
    ...
    if request.method == "POST":
        to_user = user_model.objects.get(pk=to_name)
...
def friendship_accept(request, friendship_request_id):
    ...
    if request.method == "POST":
        ...
        f_request.accept()
        return redirect("friendship:friendship_view_friends", name=request.user.name)
...
def all_users(request, template_name="friendship/user_actions.html"):
    ...
    return render(
        request, template_name, {get_friendship_context_object_list_name(): page_obj}
    )
