...
def search(request):
    page = request.GET.get('paging', '1')  # 페이지
    kw = request.GET.get('g_kw', '')  # 검색어
    so = request.GET.get('so','recent')
    ...
    context = {'post_list': page_obj, 'page': page, 'category_list': category_list, 'kw': kw, "so":so}
    return render(request, 'board/globallist.html', context)
